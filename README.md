...
The repository also includes a browser-ready Streamlit companion. It recreates the
decision-facing Power BI views from the reviewed Parquet exports, with carrier,
warehouse, and shipment-date filters. It is interactive in a browser but remains a
snapshot until the source exports are regenerated and redeployed.
...
Every number above is computed over the **complete unfiltered fact table** — all
98,595 live, non-test shipments, with no date slicer and no page filter applied
— and is regenerated into
[`reports/executive_summary.csv`](reports/executive_summary.csv) by pipeline step
11. That file is the authoritative reconciliation target: full-fill rate is
`0.813661` and gross-margin rate is `0.667930`, which round to the 81.4% and
66.8% shown here.

The dashboard captures in `images/` were exported with the report's saved
**Shipment Date** slicer state, which ends before the last shipment date, so a
card there can differ from this table in the final decimal. When a card and this
table disagree, this table and `reports/executive_summary.csv` are correct. The
capture protocol that keeps the two aligned is in
[`powerbi/REPORT_BLUEPRINT.md`](powerbi/REPORT_BLUEPRINT.md).
...
| `data/public_dashboard/` | Tracked reviewed fact, dimensions, and KPI Parquet snapshot consumed by Streamlit |
| `data/powerbi/` | Local pipeline output used to refresh Power BI; regenerated by pipeline step 10 |
| [`streamlit_app.py`](streamlit_app.py) | Browser dashboard built from the reviewed shipment-grain exports |
...
| [`images/Logistics_Control_Tower.pdf`](images/Logistics_Control_Tower.pdf) | Reviewer-friendly five-page export of the polished narrative report |
...
- The local Power BI artifact opens on **Executive Overview**. Its five public narrative pages are complemented by hidden Shipment Detail drillthrough and Data Quality diagnostic pages retained for technical review.
- A five-page recruiter-facing PDF export is available at [`images/Logistics_Control_Tower.pdf`](images/Logistics_Control_Tower.pdf) for reviewers who do not have Power BI Desktop.
- Readable public-page evidence is available in [`images/Executive_Overview.png`](images/Executive_Overview.png), [`images/Carrier_Lane_Performance.png`](images/Carrier_Lane_Performance.png), and [`images/Profitability.png`](images/Profitability.png). [`images/Data_Quality_Coverage.png`](images/Data_Quality_Coverage.png) documents the internal diagnostic page, and [`images/Model_View.png`](images/Model_View.png) documents the semantic model.
...
- The reviewed PBIX and dashboard-ready data are distributed through the [latest GitHub release](https://github.com/Akshith597/logistics-control-tower/releases/latest); the PBIX remains out of the source tree because `.pbix` is intentionally ignored.
...
- The hidden Power BI Data Quality diagnostic page keeps customer, carrier,
  warehouse, invoice, WMS, fill-rate, and survey coverage available for
  technical review without crowding the recruiter-facing export.
