...
A binary `.pbix` is not tracked in this repository. It is produced by Power BI
Desktop rather than generated by any script here, so it would be a large binary
that no test or workflow can verify. The guides above are the authoritative
build instructions; save your finished report as
`powerbi/Logistics_Control_Tower.pbix` locally. Reviewers without Power BI
Desktop can read the report design in
